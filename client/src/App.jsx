...
function App() {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/showListings`, {
          headers: {
            Authorization: `Bearer ${token}`,
          }
        });
  ...
  const handleMessageButton = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${import.meta.env.VITE_API_URL}/api/create_chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleReportSubmit = async () => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL}/api/add_report`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
          ...
          <div className="modal2-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal2-grid-container">
              ...
              <div className="listing2-photo-column">
                <img
                  src={`${import.meta.env.VITE_API_URL}${selectedListing.photo}`}
